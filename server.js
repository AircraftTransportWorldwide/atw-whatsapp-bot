// ATW WhatsApp Bot v10.30 — server.js
...
app.post('/webhook', async (req, res) => {
  ...
  if (!mem.disclaimerSent) {
    pendingDisclaimer = {
      es: 'Nota: Todas las consultas de envio estan sujetas a revision y aprobacion por parte de nuestro equipo de operaciones. La confirmacion final de rutas, precios y cumplimiento normativo es proporcionada unicamente por el personal de ATW.',
      pt: 'Nota: Todas as consultas de envio estao sujeitas a revisao e aprovacao pela nossa equipe de operacoes. A confirmacao final de rotas, precos e conformidade regulatoria e fornecida exclusivamente pela equipe ATW.',
    ...
    }[mem.language] || 'Please note: all shipment inquiries are subject to review and approval by our operations team. Final confirmation of routing, pricing, and regulatory compliance is provided by ATW staff only.';
  ...
  catch (err) { console.error('[Claude] Error:', err.message); reply = 'I\'m having a technical issue. Please call us at +1 (305) 871-1020 for immediate assistance.'; }
  if (!reply) return;
...
app.listen(PORT, () => console.log(`[Boot] ATW Bot v10.30 running on port ${PORT}`));
